...
using apollo::perception::PerceptionObstacle;//add by xuefeng
...
bool PredictionComponent::PredictionEndToEndProc(
  ...
  AERROR<<"predict module:"<<perception_msg.perception_obstacle_size();
  for (const PerceptionObstacle& perception_obstacle :
       perception_msg.perception_obstacle()){
    auto* obstacle = prediction_obstacles.add_prediction_obstacle();
    auto* perception_obstacle_temp = obstacle->mutable_perception_obstacle();
    perception_obstacle_temp->CopyFrom(perception_obstacle);
  }
  ...
  std::chrono::duration<double> diff = end_time5 - end_time1;
  ...
  //cyber::common::SetProtoToASCIIFile(prediction_obstacles, "temp.proto");
  return true;
}
