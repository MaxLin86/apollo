...
Status VehicleStateProvider::Update(
  ...
  //add by shzhw to set angular_velocity and linear_acceleration
  /*code*/
  if (FLAGS_enable_map_reference_unify) {
    ...
    vehicle_state_.set_angular_velocity(localization.pose().angular_velocity_vrf().z());  // change by why
    }
    ...
    vehicle_state_.set_linear_acceleration(localization.pose().linear_acceleration_vrf().y());  // change by why
    }
  } else {
    ...
    vehicle_state_.set_angular_velocity(localization.pose().angular_velocity().z());  // change by why
    }
    ...
    vehicle_state_.set_linear_acceleration(localization.pose().linear_acceleration().y());  // change by why
    }
  }
  ...
  if (std::abs(vehicle_state_.linear_velocity()) < kEpsilon) {
    ...
    vehicle_state_.set_kappa(vehicle_state_.angular_velocity() /
                             (vehicle_state_.linear_velocity()+1e-8));  // change by why
  }
...
bool VehicleStateProvider::ConstructExceptLinearVelocity(
  ...
  if (localization.pose().has_position()) {
    ...
    vehicle_state_.set_z(localization.pose().position().z());  // change by why
  }

  const auto &orientation = localization.pose().orientation();

  if (localization.pose().has_heading()) {
    vehicle_state_.set_heading(localization.pose().heading());  // change by why
  } else {
      vehicle_state_.set_heading(math::QuaternionToHeading(orientation.qw(), orientation.qx(),
                                                           orientation.qy(), orientation.qz()));  // change by why
  }
