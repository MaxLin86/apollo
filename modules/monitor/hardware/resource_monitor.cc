...
bool GetPIDByCmdLine(const std::string& process_dag_path, int* pid) {
  ...
  for (const auto& dir_name : dirs) {
    ...
    *pid = std::stoi(dir_name);
    ...
    AERROR<<"bkx:GetPIDByCmdLine"<<"system_proc_path:"<<system_proc_path<<"dir_name:"<<dir_name<<"proc_cmdline_path:"<<proc_cmdline_path;
...
AERROR<<"bkx:GetCPUUsage"<<"cpu_stat_file:"<<cpu_stat_file;
...
void ResourceMonitor::RunOnce(const double current_time) {
  ...
  for (const auto& iter : mode.monitored_components()) {
    ...
    AERROR<<"ResourceMonitor::RunOnce:"<<name;
  ...
  for (const auto& disk_space : config.disk_spaces()) {
    for (const auto& path : cyber::common::Glob(disk_space.path())) {
      ...
      if (available_gb < disk_space.insufficient_space_error()) {
        ...
        SummaryMonitor::EscalateStatus(ComponentStatus::ERROR, ErrorCode::MONITOR_DISKSPACE_ERROR, err, status);
      } else if (available_gb < disk_space.insufficient_space_warning()) {
        const std::string err =
            absl::StrCat(path, " has insufficient space: ", available_gb,
                         "GB < ", disk_space.insufficient_space_warning());
        SummaryMonitor::EscalateStatus(ComponentStatus::WARN, ErrorCode::MONITOR_DISKSPACE_WARN, err, status);
      }
      AERROR<<"ResourceMonitor::CheckDiskSpace path="<<path<<";available_gb"<<available_gb<<";disk_space.insufficient_space_error():"<<disk_space.insufficient_space_error()<<"disk_space.insufficient_space_warning():"<<disk_space.insufficient_space_warning();
    }
  }
}
...
void ResourceMonitor::CheckCPUUsage(
  ...
  for (const auto& cpu_usage : config.cpu_usages()) {
    ...
    if (process_dag_path.empty()) {
      ...
      AERROR<<"cpu_usage_value:"<<cpu_usage_value;
    ...
    if (cpu_usage_value > high_cpu_error) {
      const std::string err = absl::StrCat(
          ...
          high_cpu_error, "%");
      SummaryMonitor::EscalateStatus(ComponentStatus::ERROR,  ErrorCode::MONITOR_CPU_ERROR, err, status);
    } else if (cpu_usage_value > high_cpu_warning) {
      const std::string warn = absl::StrCat(
          process_dag_path, " has high cpu usage: ", cpu_usage_value, "% > ",
          high_cpu_warning, "%");
      SummaryMonitor::EscalateStatus(ComponentStatus::WARN, ErrorCode::MONITOR_CPU_WARN, warn, status);
    }
      AERROR<<"bkx:high_cpu_warning:"<<high_cpu_warning<<";high_cpu_warning:"<<high_cpu_error;
  }
}
...
void ResourceMonitor::CheckMemoryUsage(
  ...
  for (const auto& memory_usage : config.memory_usages()) {
    ...
    if (process_dag_path.empty()) {
      ...
      AERROR<<"bkx:process_dag_path:"<<process_dag_path<<";memory_usage_value:"<<memory_usage_value;
    ...
    if (memory_usage_value > static_cast<float>(high_memory_error)) {
      const std::string err = absl::StrCat(
          ...
          " > ", high_memory_error);
      SummaryMonitor::EscalateStatus(ComponentStatus::ERROR, ErrorCode::MONITOR_MEMORY_ERROR, err, status);
    } else if (memory_usage_value > static_cast<float>(high_memory_warning)) {
      const std::string warn = absl::StrCat(
          process_dag_path, " has high memory usage: ", memory_usage_value,
          " > ", high_memory_warning);
      SummaryMonitor::EscalateStatus(ComponentStatus::WARN, ErrorCode::MONITOR_MEMORY_WARN,warn, status);
    }
      AERROR<<"bkx:high_memory_warning:"<<high_memory_warning<<";high_memory_error:"<<high_memory_error;
  }
}
...
void ResourceMonitor::CheckDiskLoads(
  ...
  for (const auto& disk_load : config.disk_load_usages()) {
    ...
    if (disk_load_value > static_cast<float>(high_disk_load_error)) {
      ...
      SummaryMonitor::EscalateStatus(ComponentStatus::ERROR, ErrorCode::MONITOR_DISKLOAD_ERROR, err, status);
    } else if (disk_load_value > static_cast<float>(high_disk_load_warning)) {
      const std::string warn = absl::StrCat(
          disk_load.device_name(), " has high disk load: ", disk_load_value,
          " > ", high_disk_load_warning);
      SummaryMonitor::EscalateStatus(ComponentStatus::WARN, ErrorCode::MONITOR_DISKLOAD_WARN,warn, status);
    }
      AERROR<<"bkx:disk_load_value:"<<disk_load_value<<";high_disk_load_warning:"<<high_disk_load_warning<<";high_disk_load_error:"<<high_disk_load_error;
  }
}
