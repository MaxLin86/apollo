...
bool MonitorManager::StartFrame(const double current_time) {
  ...
  if (current_mode_ != hmi_status->current_mode()) {
    ...
    AERROR<<"bkx:MonitorManager::StartFrame mode_config_"<<hmi_config_.modes().at(current_mode_);
    ...
    for (const auto& iter : mode_config_.modules()) {
      AERROR << "MonitorManager::StartFrame-modules:" << iter.first;
    ...
    AERROR<<"bkx:MonitorManager::StartFrame iter.first"<<iter.first;
  ...
  AERROR<<"bkx:MonitorManager::StartFrame:"<<current_mode_;
