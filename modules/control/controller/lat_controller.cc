...
#include <fstream>  // add by why
#include <iostream>   // add by why
#include <algorithm>
#include <iomanip>
#include <utility>
#include <vector>
...
void LatController::ComputeLateralErrors(
  ...
  /*
 {
  ...
  location_out.open("lat_result_out.txt", std::ios::out | std::ios::app);
  location_out << lateral_error << "\t" << lateral_error_feedback <<  "\t"<< heading_error << "\t" << heading_error_feedback << "\t" << x <<"\t"<< y << "\t"<< target_point.path_point().x() << "\t"<< target_point.path_point().y() << std::endl;
  location_out.close();
  }
  */
