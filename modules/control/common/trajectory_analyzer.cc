...
TrajectoryPoint TrajectoryAnalyzer::QueryNearestPointByPosition(
  ...
  // double d_min = PointDistanceSquare(trajectory_points_.front(), x, y);  delete by why
  double d_min = PointDistanceSquare(trajectory_points_.back(), x, y);  // change by why
  size_t index_min = 0;

  for (size_t i = 1; i < trajectory_points_.size(); ++i) {
    double d_temp = PointDistanceSquare(trajectory_points_[i], x, y);
    if (d_temp < d_min && d_temp > 100.0) {  // add by why
      d_min = d_temp;
      index_min = i;
    }

    // if (d_temp < d_min) {  dele by why
    //   d_min = d_temp;
    //   index_min = i;
    // }
  }
