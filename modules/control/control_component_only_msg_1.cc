...
void ExecuteMoveAction(double &moving_distance, LocalView &local_view_t, ControlCommand &control_cmd,
            int &drive_gear_cnt, int &reverse_gear_cnt,  double &vehicle_move_length, bool &excut_completed) {
    ...
    if (moving_distance > 1e-4) {
      ...
      if (local_view_t.chassis().gear_location() !=
        ...
        if (local_view_t.chassis().gear_location() ==
          ...
          control_cmd.set_brake(50.0);
          reverse_gear_cnt += 1;
          if (reverse_gear_cnt > 60) {
            control_cmd.set_gear_location(Chassis::GEAR_NEUTRAL);
          } 
          if (reverse_gear_cnt > 120) {
    ...
    if ((moving_distance < - 1e-4 && local_view_t.chassis().gear_location() ==
      ...
      double braking_distance = 0.8535*abs_vehicle_speed - 0.01345 +0.01*std::fabs(moving_distance); //change 20201021
      AERROR << "excute.. vehicle dis:" << vehicle_move_length << " brake dis:" << braking_distance << " vel:" << abs_vehicle_speed << " msg dis:" << moving_distance;
...
bool ControlComponent::Proc() {
  ...
  static bool arrive_at_destination = false;
  static bool stop_at_destination = false;
  static double stop_at_destination_time_s = 0.0;
...

  {
    ...
    if ((pad_msg_.has_header()) && 
      ...
      if (pad_msg_.action() == DrivingAction::RESET) {
        ...
        reset_cnt++; 
        arrive_at_destination = false;
        stop_at_destination = false;
        stop_at_destination_time_s = 0.0;
      }
    ...
    if (abb_msg_.has_header() && 
     ...
     AERROR << "abb msg get--------------"; 
      ...
      if (abb_msg_.has_moving_distance() && std::fabs(abb_msg_.moving_distance()) >= 0.05 && execute_abb_second_positioning_completed) {
      ...
      if (stop_at_destination && execute_abb_second_positioning_completed) {       
        if (local_view_.chassis().header().timestamp_sec() - stop_at_destination_time_s > 1.0 &&
        local_view_.chassis().header().timestamp_sec() - stop_at_destination_time_s < 2.0) {
          abb_msg_valid = true;
          AERROR << "abb msg true.................";
        }       
	     AERROR << "abb msg get--------------" << "speed: " << local_view_.chassis().speed_mps();
       AERROR << std::fixed << "stop time: " << stop_at_destination_time_s << "cur time: " <<  local_view_.chassis().header().timestamp_sec();
      }

      if (execute_abb_second_positioning_completed) {
        if (abb_msg_.has_moving_distance() &&  
                    std::fabs(abb_moving_distance) >= 0.05 ) {
          if (abb_msg_valid) {
      ...
      AERROR << " complete: " << execute_abb_second_positioning_completed;
  ...
  if (!arrive_at_destination && !pad_msg_valid) { // no at destination, normal auto 
    ...
    // add by shzhw, ProduceControlCommand() is used to gear
      ...
      // just for test
    if (0 && local_view_.chassis().gear_location() ==
          Chassis::GEAR_DRIVE) {
      ...
      if (local_view_.chassis().speed_mps() < test_speed/3.6) {
       ...
       // control_command.set_steering_target(-2.0);
        control_command.set_steering_target(local_view_.trajectory().simple_control_cmd().steering_target());
        finish_flag = true;
      }
      if (finish_flag) {
    ...
    if (local_view_.trajectory().is_use_routing() && local_view_.chassis().gear_location() ==
      ...
      /**/
      #if 0
      #define STOP_PX 794820.740
      #define STOP_PY 2490789.88
      ...
      //      (local_view_.localization().pose().position().y()-STOP_PY) >= 0.0) { // zong yi road
      if ((local_view_.localization().pose().position().x()-STOP_PX) <= 0.0 &&
            (local_view_.localization().pose().position().y()-STOP_PY) >= 0.0) { //an qiao nei
        stop_p = true;
    ...
    int box_position_type = local_view_.trajectory().box_position();
    double dis_offset_to_centre = 0.0;
    switch (box_position_type) {
      case 0:
        dis_offset_to_centre = 0.0;
      break;
      case 1:
        dis_offset_to_centre = -2.9;
      break;
      case 2:
      dis_offset_to_centre =0.0;
      break;
      case 3:
      dis_offset_to_centre =  3.5;
      break;
      default:
        dis_offset_to_centre = 0.0;
      break;
    ...
    if (is_in_yard_valid_zone) {
      // only for heading = -1.24, M, TODO A and F
      double destination_px = local_view_.trajectory().target_position_coord().x() + 
                (4.7 + dis_offset_to_centre) * cos(-1.24);
      double destination_py = local_view_.trajectory().target_position_coord().y() + 
                (4.7 + dis_offset_to_centre) * sin(-1.24);
      if ((local_view_.localization().pose().position().x()-destination_px) >= 0.0 &&
            (local_view_.localization().pose().position().y()-destination_py) <= 0.0) {
        arrive_at_destination = true;
        AERROR << "arrive_at_destination flag: " << arrive_at_destination;
      }
    } else if (is_in_crane_valid_zone) {
	    AERROR << "3333333333";
	    AERROR << "arrive_at_destination: " << arrive_at_destination;
      // only for heading = 1.90, M, TODO A  F and heading 
      double destination_px = local_view_.trajectory().target_position_coord().x() + 
                (5.3 + dis_offset_to_centre) * cos(1.9);
      double destination_py = local_view_.trajectory().target_position_coord().y() + 
                (5.3 + dis_offset_to_centre) * sin(1.9);
      if ((local_view_.localization().pose().position().x()-destination_px) <= 0.0 &&
            (local_view_.localization().pose().position().y()-destination_py) >= 0.0) {
        arrive_at_destination = true;
        AERROR << "arrive_at_destination flag2: " << arrive_at_destination;
      }
    }
    ...
    ExecuteMoveAction(pad_moving_distance, local_view_, control_command,
           drive_gear_count, reverse_gear_count,  control_vehicle_move_length, excut_pad_move_action_completed);
    //TODO reset pad_msg_valid = false
    //----------------------------------------
  } else if (arrive_at_destination) {
    control_command.set_speed(0.0);
    control_command.set_throttle(0.0);
    control_command.set_brake(65);//65
    control_command.set_gear_location(local_view_.chassis().gear_location());
    if (std::fabs(local_view_.chassis().speed_mps()) <= 1e-4) {
      stop_at_destination = true;
      if (std::fabs(stop_at_destination_time_s) < 1e-4) {
        stop_at_destination_time_s = local_view_.chassis().header().timestamp_sec();
      }
    }
  }

  // stop at destination, wait for abb msg
  if (stop_at_destination) {
    AERROR << "valid: " << abb_msg_valid << " complete: " << execute_abb_second_positioning_completed;
    if (abb_msg_valid && !execute_abb_second_positioning_completed) {
      //if (std::fabs(abb_moving_distance) >= 0.07) {
    ...
    if (execute_abb_second_positioning_completed) {
      if (local_view_.chassis().header().timestamp_sec() - stop_at_destination_time_s > 5.0) { //no abb msg, default alignement ok
        control_command.set_speed(0.0);
        control_command.set_throttle(0.0);
        control_command.set_brake(65);//65
        control_command.set_gear_location(local_view_.chassis().gear_location());
