...
//#include "modules/rms/proto/pad_msg.pb.h"
...
class PadTerminal {
 public:
  PadTerminal() : node_(CreateNode("rms_terminal")) {}
  void init() {
    ...
    abb_writer_ = node_->CreateWriter<PadMessage>("/apollo/alignement");
  ...
  void send(int cmd_type) {
    ...
    apollo::common::util::FillHeader("abb terminal", &pad);
    abb_writer_->Write(pad);
    AINFO << "send pad_message OK";
  }

// add by shzhw, to send move distance msg
  void send_move_action_msg(){
    PadMessage pad;
 ...
 private:
  ...
  std::shared_ptr<Writer<PadMessage>> abb_writer_;
