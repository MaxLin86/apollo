...
bool ControlComponent::Proc() {
  ...
  static bool excut_pad_move_action_completed = true;
...

  {
    ...
    if ((pad_msg != nullptr) && 
              ...
              local_view_.pad_msg().header().timestamp_sec() != pad_msg->header().timestamp_sec())) {
      
      local_view_.mutable_pad_msg()->CopyFrom(pad_msg_);
      if (pad_msg->header().module_name() == "perception_camera") {
      }else {
      pad_msg_valid = true;
      }
      if (pad_msg->action() == DrivingAction::RESET) {
        pad_msg_valid = false;
      }


      if (pad_msg->has_moving_distance()) {
        pad_moving_distance =pad_msg->moving_distance();
         if (pad_msg->header().module_name() == "perception_camera") {
          pad_moving_distance -=1.5; //TODO
          pad_msg_valid = false;// test 20201021
        } else if (pad_msg->header().module_name() == "positionEst") {
          pad_moving_distance -=0.0; //TODO
        }
      }
  ...
  if (!pad_msg_valid) { //no pad msg , normal auto
    ...
    if (1) {
      ...
      if (local_view_.chassis().speed_mps() < 10.0/3.6) {
       control_command.set_brake(0);
        control_command.set_throttle(14);
        control_command.set_steering_target(0);
      } else if (local_view_.chassis().speed_mps() >= 10.0/3.6) {
        control_command.set_brake(100);
      ...
      const double  abs_vehicle_speed_test = std::fabs(local_view_.chassis().speed_mps());
        ...
        AERROR << "test: " << "diff time: " << time_diff_test  << ",speed: " << abs_vehicle_speed_test << ", length: " << d_length;
    ...
    if (local_view_.trajectory().is_use_routing() && local_view_.chassis().gear_location() ==
      ...
      if ((local_view_.localization().pose().position().x()-794634.900190429)<=0.0 &&
            (local_view_.localization().pose().position().y()-2490881.77754037) >=0.0) {
        stop_p = true;
    ...
    if (pad_moving_distance > 1e-4) {
      ...
      if (local_view_.chassis().gear_location() !=
        ...
        if (local_view_.chassis().gear_location() ==
          ...
          control_command.set_brake(control_conf_.soft_estop_brake());
          drive_gear_count += 1;
          if (drive_gear_count > 60) {
            control_command.set_gear_location(Chassis::GEAR_NEUTRAL);
            //AERROR << "SET NEUTRAL.2.. " << drive_gear_count << ", " << local_view_.chassis().gear_location();
          } 
          if (drive_gear_count > 120) {
            control_command.set_gear_location(Chassis::GEAR_DRIVE);
            //AERROR << "SET DRIVE.2.. " << drive_gear_count << ", " << local_view_.chassis().gear_location();
          }
        }
      } else if (local_view_.chassis().gear_location() ==
              Chassis::GEAR_DRIVE) {
    ...
    } else  if (pad_moving_distance < - 1e-4) {
      ...
      if (local_view_.chassis().gear_location() !=
        ...
        if (local_view_.chassis().gear_location() ==
          ...
          if (reverse_gear_count > 60) {
            control_command.set_gear_location(Chassis::GEAR_NEUTRAL);
          } 
          if (reverse_gear_count > 120) {
    ...
    // excute move action
    if ((pad_msg->moving_distance() < - 1e-4 && local_view_.chassis().gear_location() ==
             Chassis::GEAR_REVERSE) || (pad_msg->moving_distance() > 1e-4 &&local_view_.chassis().gear_location() ==
             Chassis::GEAR_DRIVE )) {
               //AERROR << "EXCUTE";
      const double vehicle_speed = local_view_.chassis().speed_mps();
      ...
      control_vehicle_move_length += time_diff * abs_vehicle_speed;
      double braking_distance = 0.8535*abs_vehicle_speed - 0.01345 +0.01*std::fabs(pad_moving_distance); //change 20201021
      if((control_vehicle_move_length + braking_distance) <= std::fabs(pad_moving_distance)) {
        control_command.set_throttle(FLAGS_drive_len_test_throttle);
        if (abs_vehicle_speed < 0.8/3.6) {
          control_command.set_throttle(12);
        } else if (abs_vehicle_speed > 3.0/3.6) {
          control_command.set_throttle(11);
        }
        if (std::fabs(control_vehicle_move_length-std::fabs(pad_moving_distance))<1.0) {
          //control_command.set_throttle(0);
        }
        control_command.set_brake(0);
        //control_command.set_steering_target(0.0);
        ...
        control_command.set_gear_location(local_view_.chassis().gear_location());
        //AERROR << "Driving ing..." << ",  abs_vehicle_speed:" << abs_vehicle_speed << " ,control_vehicle_move_length:" << control_vehicle_move_length;
      } else {
        control_command.set_speed(0.0);
        control_command.set_throttle(0.0);
    ...
    //----------------------------------------
     ...
     // excute move action completed, stop
