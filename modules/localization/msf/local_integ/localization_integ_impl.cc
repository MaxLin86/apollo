...
void LocalizationIntegImpl::GnssBestPoseProcessImpl(
    const drivers::gnss::GnssBestPose& bestgnsspos_msg) {
      ...
      msf::FrameTransform::LatlonToUtmXY(bestgnsspos_msg.longitude()/180.0*M_PI,
                                  bestgnsspos_msg.latitude()/180.0*M_PI, &utm_xy);
      cur_utm_xy.x = utm_xy.x;
      cur_utm_xy.y = utm_xy.y; 
        std::ofstream obs_gnss_out;
        obs_gnss_out.open("obs_gnss_out.txt",std::ios::app);
        obs_gnss_out <<std::fixed <<bestgnsspos_msg.header().timestamp_sec() << "\t" << cur_utm_xy.x << "\t" <<cur_utm_xy.y << "\t" << 0 << std::endl;  
