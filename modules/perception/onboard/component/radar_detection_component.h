...
class RadarDetectionComponent : public cyber::Component<ContiRadar> {
 ...
 private:
  ...
  void radarObjectFuse(const std::vector<drivers::ContiRadarObs>& Obsvector,
                       drivers::ContiRadar& tmp_message);
  void messageCollection(const std::shared_ptr<ContiRadar>& sub_message,
                          std::vector<drivers::ContiRadarObs>* Obsvector);
  void FuseMainObj(drivers::ContiRadar& message);
 ...
 private:
  ...
  float obj_match_size_;
  float obj_cluster_size_;
