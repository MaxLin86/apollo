...
bool FusionCameraDetectionComponent::DetectGuidePost(cv::Mat img, std::vector<cv::Point2f> &grd_pt) {
  ...
  cv::Rect rect_roi = cv::Rect(0, 550, 1920, 250);//start pt and width 、height
  ...
  for (int i = 0; i < ImageRoi.rows; i++) {
    for (int j = 0; j < ImageRoi.cols; j++) {
      if (channels[2].at<uchar>(i, j) *1.0 - channels[1].at<uchar>(i, j) > 15
         && channels[1].at<uchar>(i, j) *1.0 - channels[0].at<uchar>(i, j) > 15) {
        binaryImage.at<uchar>(i, j) = 255;
      }
      else {
        binaryImage.at<uchar>(i, j) = 0;
...
bool FusionCameraDetectionComponent::DetectYellowLane(cv::Mat & image, int nLinePos, float fLaneW, camera::CameraFrame *frame, double fCarSpeed, double fCarAngle) {
  ...
  if(nLinePos & 1){//1：detect left,2:detect right,3:two side
    ...
    if(left_track_status == 0){
      ...
      AERROR<<left_track_num;
    }
    
    //if(left_track_num > 6){
    if(fNoLeftLaneDis > 2.0 && left_track_num > 30){
      left_detect_num = 0;
      left_track_num = 0;
      left_track_status = 0;
    }
  ...
  if(nLinePos & 2){
    ...
    AERROR<<"fNoRightLaneDis:"<<fNoRightLaneDis;
    //if(right_track_num > 6){
    if(fNoRightLaneDis > 2.0 || right_track_num > 30){
      right_detect_num = 0;
      right_track_num = 0;
  ...
  last_time = cur_time;
  AERROR<<"left_detect_num"<<left_detect_num<<","<<"left_track_status,"<<left_track_status<<","<<"right_detect_num"<<right_detect_num<<","<<"right_track_status:"<<right_track_status;
...
bool FusionCameraDetectionComponent::DetectLane(cv::Mat & image, std::vector<cv::Vec4i> &lines_img) {
  ...
  //cv::threshold(absGradX, edgeImage, 100, 255, CV_THRESH_OTSU);//自适应二值化CV_THRESH_OTSU
 ...
 // GaussianBlur(srcImageRoi, srcImageRoi, Size(3, 3), 0, 0, BORDER_DEFAULT);
  ...
  while (iter != map_lines.end()) {
    ...
    if(gray_avr < 120){//取固定值，图像整体较暗时，会有问题
      continue;
    }
    if(FLAGS_lane_debug_display){
...
int FusionCameraDetectionComponent::InternalProc(
  ...
  if(camera_name == lane_detect_camera_){
    ...
    //double car_latitude = gnss_bestpos_.latitude();
    //double longitude = gnss_bestpos_.longitude();

    //DetectYellowLane(undistort_img, 2, 400,&camera_frame, cur_speed, cur_angle);

    // double time4 = apollo::common::time::Clock::NowInSeconds();
    // diff = time4 - time3;
    // AERROR<<"DetectLaneNew:"<<diff;
    //cv::imwrite("aige.jpg", output_image);
    //add by houxuefeng 
  }
  else{
    cv::Mat output_image(image_height_, image_width_, CV_8UC3,
  ...
  //add by houxuefeng
    ...
    cv::putText(output_image, std::to_string(camera_frame.frame_id), cv::Point(100,100),
          cv::FONT_HERSHEY_DUPLEX, 2.0, cv::Scalar(0,0,255), 3);
    
    for(auto obj:camera_frame.detected_objects){
      auto box = obj->camera_supplement.box;
      if(box.xmin < box.xmax && box.ymin < box.ymax){
        const cv::Rect detrect(box.xmin, box.ymin,
              box.xmax-box.xmin, box.ymax-box.ymin);
        cv::rectangle(output_image, detrect, cv::Scalar(255, 0, 0), 2);
      }
    }
