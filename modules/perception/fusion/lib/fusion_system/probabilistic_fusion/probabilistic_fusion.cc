...
bool ProbabilisticFusion::Fuse(const FusionOptions& options,
  ...
  //add by xuefeng
  if(0)//sensor_data_manager->IsCamera(sensor_frame))
  {
    cv::Mat output_image(1080, 1920, CV_8UC3,
                       cv::Scalar(250, 0, 0));
  base::Image8UPtr out_image;
  out_image = sensor_frame->camera_frame_supplement.image_ptr;
...
camera_name = sensor_frame->sensor_info.name;
  for (auto obj : sensor_frame->objects) {
