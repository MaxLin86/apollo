...
void DummyDetector::ContiObs2Frame(
  ...
  for (const auto& radar_obs : corrected_obstacles.contiobs()) {
    ...
    radar_object->center(0) = radar_obs.pos_x();
    radar_object->center(1) = radar_obs.pos_y();
    radar_object->center(2) = radar_obs.pos_z();
    ...
    radar_object->velocity(0) = static_cast<float>(radar_obs.doppler());
    radar_object->velocity(1) = 0.0f;
    radar_object->velocity(2) = 0.0f;
