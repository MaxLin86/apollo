...
bool Frame::CreateReferenceLineInfo(
  ...
  //add by shzhw, set different cruise_speed
  static double default_cruise_speed = FLAGS_default_cruise_speed;
  if(null_collision_obstacle_) {
    FLAGS_default_cruise_speed = default_cruise_speed;
  } else {
      FLAGS_default_cruise_speed = 1.3;
  }    
  //---------------------------------------------
...
//add by shzhw, to init static variate
uint32_t Frame:: null_collision_obstacle_cycs_ = 0;
bool Frame::null_collision_obstacle_ = true;
//-------------------------------------
...
Status Frame::InitFrameData() {
  ...
  // AERROR << "k = " << k;//test by shzhw
  //add by shzhw, add virutal obstacles 
  if (FLAGS_enable_create_virtual_obstalce) { //just for simulation test
    static double pre_timestamp = vehicle_state_.timestamp();
    static double drive_path_length = 0.0;
    double time_diff = vehicle_state_.timestamp() - pre_timestamp;
    // AERROR << "TIME DIFF: " << time_diff;
    const double vehicle_speed = std::max(std::fabs(vehicle_state_.linear_velocity()),0.5);
    drive_path_length += time_diff * vehicle_speed;
    pre_timestamp = vehicle_state_.timestamp();
    double virtual_obstacle_px = FLAGS_virtual_obstacle_range - drive_path_length;
    virtual_obstacle_px = virtual_obstacle_px > 3.0? virtual_obstacle_px:1.0;

    perception::PerceptionObstacle perception_obstacle;
    std::map<std::string, Obstacle> obstacle_buf;
    std::vector<const Obstacle*> obstacles;

    perception_obstacle.mutable_position()->set_x(virtual_obstacle_px); 
    ...
    std::string id = "created virtual obstacle for stopping";
    obstacle_buf.emplace(
        id, Obstacle(id, perception_obstacle, prediction::ObstaclePriority::NORMAL, false));
    obstacles.emplace_back(&obstacle_buf[id]);
    AddObstacle(obstacle_buf[id]);
 
    /*
    if(perception_obstacle.position().x()<30)
    {
      std::string err_str =
          "Found too collision with obstacle: " ;
      AERROR << err_str;
  ...
  bool has_near_obstacle = false;
  static double nearest_obstacle_px = 300.0;
  static double nearest_obstacle_py = 30.0;
  // int kk=0;
  for (const auto &obstacle : obstacles_.Items()) {
    // kk++;
    const double obstacle_px = obstacle->Perception().position().x();
    const double obstacle_py = obstacle->Perception().position().y();
    if (obstacle_px <= FLAGS_obstacle_lon_distance_threshold_to_stop 
        && obstacle_px > 0 && 
        std::fabs(obstacle_py) < FLAGS_obstacle_lat_distance_threshold_to_stop) {
      has_near_obstacle = true;
      null_collision_obstacle_cycs_ = 0;    
      null_collision_obstacle_ = false;
      //(to do) just simpleness
      if(obstacle_px < nearest_obstacle_px) {
        nearest_obstacle_px = obstacle_px;
        nearest_obstacle_py = obstacle_py;
      }
      //break;
    }      
  }
  // AERROR << "kk= " << kk; //test by shzhw
  if (!has_near_obstacle) {
    null_collision_obstacle_cycs_++;
    if(null_collision_obstacle_cycs_> FLAGS_not_detect_obstacle_count_threshold){
        null_collision_obstacle_ = true;
    }  
  }

  if (!null_collision_obstacle_ && !has_near_obstacle) { // add a virtual obstacle
  //FLAGS_default_cruise_speed = 5.0; //invaild
  nearest_obstacle_px -= FLAGS_default_obstacle_ref_vx * 0.1;
  nearest_obstacle_px = nearest_obstacle_px >3.0? nearest_obstacle_px:1.0;
  /*
  static double px0  = 30;
  px0 -=0.05;
  px0 = px0 > 3?px0 :0.1;
  */
  perception::PerceptionObstacle virtual_predict_perception_obstacle;
  std::map<std::string, Obstacle> virtual_predict_obstacle_buf;
  // std::vector<const Obstacle*> virtual_predict_obstacles;
  virtual_predict_perception_obstacle.mutable_position()->set_x(nearest_obstacle_px);
  virtual_predict_perception_obstacle.mutable_position()->set_y(nearest_obstacle_py);
  virtual_predict_perception_obstacle.mutable_velocity()->set_x(0.0);
  virtual_predict_perception_obstacle.mutable_velocity()->set_y(0.0);
  virtual_predict_perception_obstacle.set_length(3.0);
  virtual_predict_perception_obstacle.set_width(3.0);
  
  std::string id = "virtual predict obstacle 1";
  virtual_predict_obstacle_buf.emplace(
    id, Obstacle(id, virtual_predict_perception_obstacle, prediction::ObstaclePriority::NORMAL, false));
  // virtual_predict_obstacles.emplace_back(&virtual_predict_obstacle_buf[id]);
  AddObstacle(virtual_predict_obstacle_buf[id]);
  }
...
//add by shzhw
uint32_t Frame::NullCollisionObstacleCycs() const { return null_collision_obstacle_cycs_; }
bool Frame::NullCollisionObstacle() const{ return null_collision_obstacle_; }
//---------------------------------
