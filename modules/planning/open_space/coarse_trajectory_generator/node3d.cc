...
Node3d::Node3d(double x, double y, double phi, const double trailer_phi) {
  x_ = x;
  y_ = y;
  phi_ = phi;
...
Node3d::Node3d(const std::vector<double>& traversed_x,
  ...
  CHECK_EQ(traversed_x.size(), traversed_trailer_phi.size());
...
/**/
Box2d Node3d::GetTrailerBoundingBox(const common::VehicleParam& vehicle_param_,
                             const double x, const double y, const double phi) {
  double ego_length = 9.50;
