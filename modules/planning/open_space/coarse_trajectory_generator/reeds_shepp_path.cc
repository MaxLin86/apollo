...
bool ReedShepp::ShortestRSP(const std::shared_ptr<Node3d> start_node,
  ...
  (*optimal_path).phi = all_possible_paths[optimal_path_index].phi;
    (*optimal_path).trailer_phi = all_possible_paths[optimal_path_index].trailer_phi;// add 0724
...
bool ReedShepp::GenerateLocalConfigurations(
  ...
  while (std::fabs(px.back()) < epsilon && std::fabs(py.back()) < epsilon &&
    ...
    pgear.pop_back();
         ...
         #if 0
          double theta_offset  = start_node->GetPhi() - start_node->GetTrailerPhi();
         double pre_tractor_tehta = start_node->GetPhi() ;
         #endif

         //method 2
  double trailer_orient_last_time = common::math::WrapAngle(start_node->GetTrailerPhi());
  double vehicle_and_trailer_angle_last =  common::math::WrapAngle(start_node->GetPhi()) - common::math::WrapAngle(start_node->GetTrailerPhi());
  //---------------------------
  ...
  for (size_t i = 0; i < px.size(); ++i) {
    ...
    //shortest_path->phi.push_back(pphi.at(i) + start_node->GetPhi());

   /* 
    static int aa = 1;

    if( aa  >= 58384 && px.size() == 53 && std::fabs(shortest_path->x[shortest_path->phi.size()-1] + 7.38) < 0.1) {
        std::cout << "-----------" << aa << std::endl;
        std::cout << shortest_path->x[shortest_path->phi.size()-1] << ", " << shortest_path->y[shortest_path->phi.size()-1] << std::endl;
    }
    aa++;
    */
        //static double theta_offset = 0.0;      
       //  static double theta_offset  = start_node->GetPhi() - start_node->GetTrailerPhi();
       // static double pre_tractor_tehta = shortest_path->phi[0];
       #if 0
         if( i == 0) {
             shortest_path->trailer_phi.push_back(
                    common::math::NormalizeAngle(start_node->GetTrailerPhi()));
         } else if (i > 0) {
           double cur_tractor_theta = shortest_path->phi[shortest_path->phi.size()-1];
           double tractor_theta_offset = cur_tractor_theta - pre_tractor_tehta;
           pre_tractor_tehta = cur_tractor_theta;

            double   ds =sqrt((shortest_path->x[shortest_path->phi.size()-1]-shortest_path->x[shortest_path->phi.size()-2])*(shortest_path->x[shortest_path->phi.size()-1]-shortest_path->x[shortest_path->phi.size()-2]) +
             (shortest_path->y[shortest_path->phi.size()-1]-shortest_path->y[shortest_path->phi.size()-2])*(shortest_path->y[shortest_path->phi.size()-1]-shortest_path->y[shortest_path->phi.size()-2]))*cos(cur_tractor_theta);

            double delta_theta_offset = ds/12.0*sin(theta_offset) + tractor_theta_offset;
            theta_offset = theta_offset + delta_theta_offset;
            double trailer_theta = cur_tractor_theta - theta_offset;
            theta_offset = cur_tractor_theta - trailer_theta;
                    shortest_path->trailer_phi.push_back(
                common::math::NormalizeAngle(trailer_theta)); // add 0724//////////////////////////////////////////
         }
         #endif

         //method 2
         static double trailer_wheelbase = 12.0;
         if ( i == 0) {
            shortest_path->trailer_phi.push_back(
                   common::math::NormalizeAngle(start_node->GetTrailerPhi()));
            //shortest_path->trailer_phi.push_back(start_node->GetTrailerPhi());
         } else {
           const double vehicle_orient_cur_time = common::math::WrapAngle(shortest_path->phi[shortest_path->phi.size()-1]);
           //const double trailer_orient_last_time = shortest_path->trailer_phi[shortest_path->trailer_phi.size()-1];
           double vehicle_and_trailer_angle_temp = vehicle_orient_cur_time - trailer_orient_last_time;
           //double vehicle_and_trailer_angle_last = shortest_path->phi[shortest_path->phi.size()-2] - trailer_orient_last_time;//---------
           double vehicle_and_trailer_angle_new = (vehicle_and_trailer_angle_last + vehicle_and_trailer_angle_temp) / 2;
           double trailer_junction_radius = trailer_wheelbase / sin(vehicle_and_trailer_angle_new);
           double   rear_wheel_center_arc_len =sqrt((shortest_path->x[shortest_path->x.size()-1]-shortest_path->x[shortest_path->x.size()-2])*(shortest_path->x[shortest_path->x.size()-1]-shortest_path->x[shortest_path->x.size()-2]) +
             (shortest_path->y[shortest_path->y.size()-1]-shortest_path->y[shortest_path->y.size()-2])*(shortest_path->y[shortest_path->y.size()-1]-shortest_path->y[shortest_path->y.size()-2]));

           double trailer_longitudinal_base_last_time = rear_wheel_center_arc_len / trailer_junction_radius;
           double trailer_orient_cur_time = trailer_orient_last_time + trailer_longitudinal_base_last_time;
           vehicle_and_trailer_angle_new = vehicle_orient_cur_time - trailer_orient_cur_time;
            trailer_orient_last_time = trailer_orient_cur_time;
	          vehicle_and_trailer_angle_last = vehicle_and_trailer_angle_new;
            shortest_path->trailer_phi.push_back(
                common::math::NormalizeAngle(trailer_orient_cur_time));
            //shortest_path->trailer_phi.push_back(trailer_orient_cur_time); // add 0724//////////////////////////////////////////
         }

        
/*
          std::ofstream node_out2;
  node_out2.open( "node_out2.txt",std::ios::app);
  node_out2 << px.size() << "\t" <<shortest_path->x[shortest_path->phi.size()-1]<< "\t" <<shortest_path->y[shortest_path->phi.size()-1] << "\t" << shortest_path->phi[shortest_path->phi.size()-1] << "\t" << shortest_path->trailer_phi[shortest_path->phi.size()-1] << std::endl; 
