...
bool HybridAStar::RSPCheck(
  ...
  std::shared_ptr<Node3d> node = std::shared_ptr<Node3d>(new Node3d(
      reeds_shepp_to_end->x, reeds_shepp_to_end->y, reeds_shepp_to_end->phi,  reeds_shepp_to_end->trailer_phi, 
      XYbounds_, planner_open_space_config_)); // add 0724
...
bool HybridAStar::ValidityCheck(std::shared_ptr<Node3d> node) {
  ...
  const auto& traversed_phi = node->GetPhis();
    ...
    const auto& traversed_trailer_phi = node->GetTrailerPhis();
/*
  std::ofstream node_out;
  node_out.open( "node_out.txt",std::ios::app);
  node_out << node_step_size << "\t" << traversed_x[0] << "\t" << traversed_y[0] << "\t" << traversed_phi[0] << "\t" << traversed_trailer_phi[0] << "\t" 
                                                                              << traversed_x[1] << "\t" << traversed_y[1] << "\t" << traversed_phi[0]  << "\t" << traversed_trailer_phi[1] << std::endl; 
  node_out.close();*/
  ...
  for (size_t i = check_start_index; i < node_step_size; ++i) {
    ...
    // tractor and trailer theta <=  pi/2
    /*
    if (std::fabs(traversed_trailer_phi[i] - traversed_phi[i]) > M_PI_2) {
      return false;
    }
    */
    ...
    if (i >= 1) {
      double discrete_v = 
                        ((traversed_x[i] - traversed_x[i - 1]) / delta_t_) *
                            std::cos(traversed_phi[i]) + 
                        ((traversed_y[i] - traversed_y[i - 1]) / delta_t_) *
                            std::sin(traversed_phi[i]);
      if (discrete_v < 0) {
        return false;
      }
    }
    ...
    double vehicle_theta = -traversed_phi[i];
    double junction_pos_x = cos(vehicle_theta)*(0.4) + traversed_x[i];
    ...
    for (const auto& obstacle_linesegments : obstacles_linesegments_vec_) {
      for (const common::math::LineSegment2d& linesegment :
        ...
        if (bounding_box.HasOverlap(linesegment) || trailer_bounding_box.HasOverlap(linesegment)/**/) {
...
std::shared_ptr<Node3d> HybridAStar::Next_node_generator(
  ...
  double max_steer_angle_2 = max_steer_angle_ *1.0;//------------------------
  if (next_node_index < static_cast<double>(next_node_num_) / 2) {
    steering =
        -max_steer_angle_2 +
        (2 * max_steer_angle_2 / (static_cast<double>(next_node_num_) / 2 - 1)) *
 ...
 // steering = steering*1.5; //------------------------------------------------
  ...
  for (size_t i = 0; i < arc / step_size_; ++i) {
    ...
    theta_offset = next_phi - trailer_theta;
        ...
        const double next_trailer_phi = common::math::NormalizeAngle(
        trailer_theta); // add 0724 ////////////////////////////
        static double aa =0.0;
        aa =aa+1.0;
         //const double next_trailer_phi = trailer_theta; ////////////////////////////
            intermediate_trailer_phi.push_back(next_trailer_phi);
      #endif

      //method 2
      //double vehicle_and_trailer_angle_temp = common::math::WrapAngle(next_phi) - common::math::WrapAngle(last_trailer_phi);
      //double vehicle_and_trailer_angle_last = common::math::WrapAngle(last_phi)-  common::math::WrapAngle(last_trailer_phi);
      ...
      double next_trailer_phi = common::math::NormalizeAngle(last_trailer_phi + trailer_longitudinal_base_last_time);
      intermediate_trailer_phi.push_back(next_trailer_phi);
      //-------------------------------------
    ...
    last_y = next_y;
    last_phi = next_phi;
        last_trailer_phi = next_trailer_phi;
  }
  ...
  std::shared_ptr<Node3d> next_node = std::shared_ptr<Node3d>(
      new Node3d(intermediate_x, intermediate_y, intermediate_phi, intermediate_trailer_phi, XYbounds_,
...
bool HybridAStar::GetResult(HybridAStartResult* result) {
  ...
  std::vector<double> hybrid_a_phi;
    std::vector<double> hybrid_a_trailer_phi;
  while (current_node->GetPreNode() != nullptr) {
    ...
    std::vector<double> phi = current_node->GetPhis();
        std::vector<double> trailer_phi = current_node->GetTrailerPhis();
  ...
  hybrid_a_phi.push_back(current_node->GetPhi());
    hybrid_a_trailer_phi.push_back(current_node->GetTrailerPhi());
...
bool HybridAStar::Plan(
  ...
  start_node_.reset(
      new Node3d({sx}, {sy}, {sphi}, {sphi}, XYbounds_, planner_open_space_config_));
  end_node_.reset(
      new Node3d({ex}, {ey}, {ephi}, {ephi}, XYbounds_, planner_open_space_config_));
