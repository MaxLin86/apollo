...
class Node3d {
 public:
  Node3d(const double x, const double y, const double phi, const double trailer_phi); //add 0724
  Node3d(const double x, const double y, const double phi, const double trailer_phi,
         const std::vector<double>& XYbounds,
         const PlannerOpenSpaceConfig& open_space_conf);//add 0724
  Node3d(const std::vector<double>& traversed_x,
         const std::vector<double>& traversed_y,
         const std::vector<double>& traversed_phi,
         const std::vector<double>& traversed_trailer_phi,
         const std::vector<double>& XYbounds,
         const PlannerOpenSpaceConfig& open_space_conf);//add 0724
  ...
  static apollo::common::math::Box2d GetTrailerBoundingBox(
      const common::VehicleParam& vehicle_param_, const double x,
      const double y, const double phi);
  ...
  double GetPhi() const { return phi_; }
    double GetTrailerPhi() const { return trailer_phi_; } //add 0724
  ...
  const std::vector<double>& GetPhis() const { return traversed_phi_; }
    const std::vector<double>& GetTrailerPhis() const { return traversed_trailer_phi_; }
