...
void SimControl::RunOnce() {
  ...
  //change by shzhw
  if (FLAGS_use_sim_chassis) {
    PublishChassis(trajectory_point.v(), gear_position);
  }  
  if (FLAGS_use_sim_localization) { 
    if(FLAGS_enable_radar_write_localization_msg) {
      AERROR << "localization message is already writeed by radar module.";
      return;
    } else {
...
void SimControl::PublishDummyPrediction() {
  if(!FLAGS_use_sim_prediction) { //add by shzhw
    return;
  }
