  ...
  if (config_.lane_source() == NavigationLaneConfig::OFFLINE_GENERATED &&
    ...
    for (int i = 0; i < navigation_line_num; ++i) {
      ...
      if (ConvertNavigationLineToPath(i, path)) {
        ...
        AERROR << "5555555555555555555555555";
      }
    }
...
bool NavigationLane::ConvertNavigationLineToPath(const int line_index,
  ...
  auto enu_to_flu_func = [this, dx, dy](const double enu_x, const double enu_y,
    ...
    if (flu_x != nullptr && flu_y != nullptr) {
      /*
      // add by shzhw 20200608, record localization heading offset pi/2
           double heading_theta = original_pose_.heading();
          heading_theta -= M_PI_2;
          if (heading_theta < -M_PI) {
            heading_theta += 2.0*M_PI;
          }

      
       Eigen::Vector2d flu_coordinate = common::math::RotateVector2d(
           {enu_x + dx, enu_y + dy}, -heading_theta);
      //----------------------------------------------------------------
      */
       Eigen::Vector2d flu_coordinate = common::math::RotateVector2d(
  ...
  auto gen_navi_path_loop_func =
      [this, &navigation_path, &enu_to_flu_func](
        ...
        for (int i = start; i < end; ++i) {
          ...
          /*
          double heading_theta = flu_theta;
          heading_theta += M_PI_2;
          if (heading_theta > M_PI) {
            heading_theta -= 2.0*M_PI;
          }
          point->set_theta(heading_theta);
          */
...
ProjIndexPair NavigationLane::UpdateProjectionIndex(const common::Path &path,
  ...
  auto check_distance_func = [this, &path, &path_size](
    ...
    /*
          ...
          double cos_angle = std::cos(heading_theta);
          double sin_angle = std::sin(heading_theta);
    */
  ...
  for (int i = current_project_index; i + 1 < path_size; ++i) {
    ...
    const double kMaxDistance = 50.0; // modied by shzhw 0608, default 50.0
...
bool NavigationLane::ConvertLaneMarkerToPath(
  ...
  static double total_driving_length = 0.0;
  static double pre_timestamp = VehicleStateProvider::Instance()->timestamp();
  double cur_timestamp = VehicleStateProvider::Instance()->timestamp();
  total_driving_length += (cur_timestamp - pre_timestamp) * current_speed;
  ...
  double path_c2 = 0.0;
  double path_c3 = 0.0;
  bool guidepost_marker_valid = false;
  // add by shzhw,20200816
  // guidepost marker is valid
  //if (FLAGS_guidepost_localization_valid2) {
    if (perception_guidepost_marker_.view_range() > 5.0) {
      path_c0 = perception_guidepost_marker_.c0_position();
      path_c1 = perception_guidepost_marker_.c1_heading_angle();
      path_c2 = perception_guidepost_marker_.c2_curvature();
      path_c3 = perception_guidepost_marker_.c3_curvature_derivative();
      guidepost_marker_valid = true;
    }
  //} 
  //perceived lane marker is valid
  
  
    double lanemarker_path_c0 = (left_lane.c0_position() + right_lane.c0_position()) / 2.0;
    ...
    double  lanemarker_path_c1 = (left_lane.c1_heading_angle() * left_quality +
                      right_lane.c1_heading_angle() * right_quality) /
                    quality_divider;
    double  lanemarker_path_c2 = (left_lane.c2_curvature() * left_quality +
                      right_lane.c2_curvature() * right_quality) /
                    quality_divider;

    double  lanemarker_path_c3 = (left_lane.c3_curvature_derivative() * left_quality +
                      right_lane.c3_curvature_derivative() * right_quality) /
                    quality_divider;

    if (guidepost_marker_valid) {
      if (left_lane.view_range() > 1.0 && right_lane.view_range() > 1.0) {
    path_c0 = (path_c0 + lanemarker_path_c0) / 2.0;
    path_c1 = (path_c1 + lanemarker_path_c1) / 2.0;
    //path_c2 = (path_c2 + lanemarker_path_c2) / 2.0; lane marker c2 and c3 is invalid
    //path_c3 = (path_c3 + lanemarker_path_c3) / 2.0;
      }
    } else {
      path_c0 = lanemarker_path_c0;
      path_c1 = lanemarker_path_c1;
      path_c2 = lanemarker_path_c2;
      path_c3 = lanemarker_path_c3;
      std::cout << lane_marker_miss_count_<< std::endl;
  ...
  /*} else if (!guidepost_marker_valid) {
    AERROR << "no valid guidepost marker or lane marker exist.";
    return false;
  ...
  for (double z = start_s; z <= path_range; z += unit_z) {
    ...
    double y1 = 0;
    if (perception_guidepost_marker_.view_range() > 5.0 ||
        left_lane.view_range() > config_.min_view_range_to_use_lane_marker() ||
