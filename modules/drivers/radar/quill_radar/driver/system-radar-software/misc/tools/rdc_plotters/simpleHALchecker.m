...
switch inputStr
    case 'create'
        ...
        %[targData_lin.spRDC3, noisemean_dB.spRDC3] = checkHALdata([goldenDataPath 'scan_000000_spsuml.bin']);
        [targData_lin.stslice, noisemean_dB.stslice] = checkHALdata([goldenDataPath 'scan_000000_stslice.bin']);
        [targData_lin.hist, noisemean_dB.hist] = checkHALdata([goldenDataPath 'scan_000000_histograms.bin']);
    ...
    case 'rdc1'
        ...
        gold_noisemean_dB_RDC1 = struct2cell(gold.noisemean_dB.RDC1);
        noisemean_dB_RDC1 = struct2cell(noisemean_dB.RDC1);
        gold_targData_lin_RDC1 = struct2cell(gold.targData_lin.RDC1);
        targData_lin_RDC1 = struct2cell(targData_lin.RDC1);
        %if(all(abs(gold_noisemean_dB_RDC1 - noisemean_dB_RDC1)<=noiseDeltaThresh_dB)...
                %& all(reshape(abs(gold.targData_lin.RDC1 - targData_lin.RDC1),1,[])<=sigDeltaThresh_lin))
        if (all(abs(gold_noisemean_dB_RDC1{1} - noisemean_dB_RDC1{1})<=noiseDeltaThresh_dB)...
                && all(reshape(abs(gold_targData_lin_RDC1{1} - targData_lin_RDC1{1}),1,[])<=sigDeltaThresh_lin))
            Pass = true;
        else
            Pass = false;
        end
