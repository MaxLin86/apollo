...
#if defined(_MSC_VER)
#pragma warning(disable : 4996) // use of posix strdup, fileno, and unlink
#endif
#if _WIN32
...
#if _UNICODE
        wchar_t wtext[1024];
        mbstowcs(wtext, session_folder, strlen(session_folder)+1);//Plus null
        LPWSTR ptr = wtext;
...
public:
        ...
        , scan_start_pos(0)
    {
        ...
        else
        {
            ...
            if (hdr.magic_value != uint32_t(MAGIC_VALUE) ||
                hdr.file_format_version != uint32_t(FILE_FORMAT_VERSION))
            {
    ...
    uint32_t get_scan_write_count() const { return scan_write_count; }
    ...
    virtual size_t begin_read_scan_data_type(const char* data_type_name)
    {
        ...
        while ((uint64_t)ftello(fp) < cur_scan_hdr.next_scan_start_pos)
        {
            ...
            if (!strcmp(cur_dtype_hdr.dtype_name, data_type_name))
            {
                ...
                return size_t(cur_dtype_hdr.dtype_size_bytes);
            }
            else
            {
