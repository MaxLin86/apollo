...
plst = {'mesh','meshc','wire',...
        'surf','surfc','surfn','surfcn','contour','alphasurf','off'};
...
p.gridscale         = [1 1];    % 40x scaling for smooth grid interpolation
...
p.interpmethod      = 'spline';    % bicubic interpolation
...
switch p.plottype
   ...
   case 'mesh',    mesh (Xi,Yi,Zi,Ci,'LineWidth',1.5,'EdgeColor','interp');
   case 'surf',    surf (Xi,Yi,Zi,Ci,'EdgeColor','flat');
   ...
   case 'contour', contour(Xi,Yi,Zi,p.contourlines);
    ...
    case 'alphasurf'
        ...
        %         tz=tz-min(tz(:));
        %         tz=tz./max(tz(:));
        %         surface(Xi,Yi,Zi,Ci,'LineStyle','none','Facecolor','interp','FaceAlpha','interp','AlphaData',tz);
        mesh(Xi,Yi,Zi,Ci,'LineStyle','-','LineWidth',1,'MeshStyle','row','Facecolor','none','Edgecolor',[0 0 0],'EdgeAlpha','interp','AlphaData',tz);
        hold on
        surface(Xi,Yi,ones(size(Zi))*median(Zi(:)),Ci,'LineStyle','none','Facecolor','interp','FaceAlpha','interp','AlphaData',tz);
