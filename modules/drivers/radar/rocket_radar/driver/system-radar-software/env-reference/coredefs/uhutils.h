...
uint32_t uh_read_clock(void);                    /*! returns wall time with resolution of 1us */
uint64_t uh_read_64bit_counter(void);
void     uh_wait_us(uint32_t wait);              /*! wait (blocking) for wait usec */

uint32_t uh_read_raw32_clock_ticks(void);
uint64_t uh_read_raw64_clock_ticks(void);
uint32_t uh_ticks_to_usec(uint32_t ticks);
uint64_t uh_ticks_to_usec(uint64_t ticks);
void     uh_get_ticks_dividers(uint32_t& numerator, uint32_t& denominator);
