...
public:
    ...
    virtual ~ActivationIterator() {}

    //! returns the next sample above the specified SNR threshold
    //! returns true if output data is valid. returns false if no more points
    //! remain above this threshold.
    //!
    //! @param[out] snr          magnitude SNR in dB
    //! @param[out] range        meters from the radar
    //! @param[out] doppler      mps relative to estimated ground
    //! @param[out] azimuth      degrees from boresight, +Y is right
    //! @param[out] elevation    degrees from boresight, +Z is down
    virtual bool get_sample(float& dbFS, float& range, float& doppler, float& azimuth, float& elevation) = 0;
...
public:
    ...
    //! In practice, only the THRESH_HI bucket will contain RDC3 activations for
    //! dynamic objects which have energy above the PFA activation threshold
    ...
    //! returns an iterator which returns finalized RDC3 samples which are above
    //! the specified SNR threshold (magnitude above the noise floor)
    virtual ActivationIterator* make_iterator(float thresh_snr) const = 0;
