...
#include "modules/drivers/radar/uhnder_radar/driver/system-radar-software/engine/common/eng-api/diag-desc.h"
#include <assert.h>
...
public:
    ...
    //! manually process UhDP packets that have arrived from the Sabine radar
    ...
    //! instance can be registered at a time, the radar deletes any previous
    //! references when a new instance is registered.
    ...
    //! scan interval. This re-synchronizes the radar’s scan interval base time
    ...
    //! to run scans for this version of the Sabine Remote API
    ...
    //! Configure the static and moving thresholds for 1D and 2D scans. This API
    //! can be called at any time to change the detection thresholds, even while
    //! scanning
    virtual void        configure_detection_thresholds(const UhdpThresholdControl& thresh_ctrl) = 0;

    //! Configure adaptive thresholding on the radar. Note this feature is
    //! only enabled when SLR has been disabled (by setting the base thresholds
    //! to 0dB)
    virtual void        configure_adaptive_thresholds(const RDC_AdaptiveThreshold& static_thresh,
            const RDC_AdaptiveThreshold& moving_thresh, bool twoD) = 0;
    ...
    //! released or until a subsequent call to this function.
    virtual const uint32_t* get_hardware_unit_core_dump(const char* hw_unit_name, uint32_t& num_addr_value_tuples) = 0;
