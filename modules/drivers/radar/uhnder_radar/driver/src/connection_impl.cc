...
#include "modules/drivers/radar/uhnder_radar/driver/src/srs-profiling.h"
#include "modules/drivers/radar/uhnder_radar/driver/src/connection_impl.h"
#include "modules/drivers/radar/uhnder_radar/driver/src/scanning_impl.h"
#include "modules/drivers/radar/uhnder_radar/driver/src/scanobject_impl.h"
#include "modules/drivers/radar/uhnder_radar/driver/system-radar-software/engine/scp-src/diag/api/diag_calibrate_structs.h"
...
#include "modules/drivers/radar/uhnder_radar/driver/system-radar-software/engine/scp-src/eng-api/state-manager.h"
...
#include "modules/drivers/radar/uhnder_radar/driver/include/vehiclecan.h"

/* these must be included in a particular order, let sleeping dogs lie */
#include "modules/drivers/radar/uhnder_radar/driver/src/uhdp/discovery-diag.h"
#include "modules/drivers/radar/uhnder_radar/driver/src/uhdp/discovery-logmsg.h"
#include "modules/drivers/radar/uhnder_radar/driver/src/uhdp/discovery-antcfg.h"
#include "modules/drivers/radar/uhnder_radar/driver/src/uhdp/discovery-ppa.h"
#include "modules/drivers/radar/uhnder_radar/driver/src/uhdp/logaggr.h"
...
void Connection_Impl::allocate_protocols()
{
    ...
    PPADiscHandler*    ppa_disc = new PPADiscHandler();
    ...
    uhdp_table[UHDP_TYPE_LOG_TASK_NAME]     = ppa_disc;
...
void Connection_Impl::send_uhdp(UhdpHeader* hdr)
{
    ...
    else
    {
        ...
        printf("Dropping outgoing UHDP message of type %d (%02X), message length 0\n", hdr->message_type, hdr->message_type);
    }
}
...
int Connection_Impl::send_hello(uint32_t radar_ip, uint32_t local_ip, uint32_t timeout_sec, uint32_t uhdp_ver)
{
    ...
    msg->logger_ip_address = ntohl(local_ip);
...
bool            Connection_Impl::configure_data_capture(const UhdpCaptureControl& cap_ctrl)
{
    ...
    // Filter the enable_mask to only the data that this library currently
    // supports. There is no point in capturing data we cannot represent to the
    // user or write to file
    cap->enable_mask &= (DL_ADC | DL_SCAN_CORE_DUMP | DL_PROC_CORE_DUMP | DL_CI | DL_SS |
                         DL_DET | DL_ZERO_DOPPLER | DL_RDC1 | DL_RDC2 | DL_RDC3 | DL_HISTOGRAM |
                         DL_COV | DL_SVD | DL_MUSIC);
...
void            Connection_Impl::configure_adaptive_thresholds(
        const RDC_AdaptiveThreshold& static_thresh,
        const RDC_AdaptiveThreshold& moving_thresh,
        bool twoD)
{
    // ensure compatibility
    assert(get_diag_test_api_version("std_rdc", "set_adaptive_thresholds") == 0);

    struct set_adaptive_thresholds_input
    {
        RDC_AdaptiveThreshold static_thresh;
        RDC_AdaptiveThreshold moving_thresh;
        uint32_t    two_D_flags;
    } input;

    input.static_thresh = static_thresh;
    input.moving_thresh = moving_thresh;
    input.two_D_flags = twoD ? 1 : 0;

    // blocking wait for diag completion
    run_diag_by_name("std_rdc", "set_adaptive_thresholds", &input, sizeof(input), NULL, 0);
}


void            Connection_Impl::configure_detection_thresholds(const UhdpThresholdControl& thresh_ctrl)
{
    char buf[sizeof(UhdpHeader) + sizeof(UhdpThresholdControl)];
    UhdpHeader* hdr = (UhdpHeader*)buf;

    hdr->message_type = UHDP_TYPE_THRESHOLD;
    hdr->total_length = sizeof(buf);
    memcpy(hdr + 1, &thresh_ctrl, sizeof(UhdpThresholdControl));
...
const uint32_t* Connection_Impl::get_hardware_unit_core_dump(const char* hw_unit_name, uint32_t& num_addr_value_tuples)
{
    UhdpHeader*             uhdp = (UhdpHeader*)lld_core_dump_msg_buffer;
    UhdpLLDCoreDumpRequest* msg  = (UhdpLLDCoreDumpRequest*)(uhdp + 1);
    ...
    memset(msg->hardware_unit_name, 0, sizeof(msg->hardware_unit_name));
    strncpy(msg->hardware_unit_name, hw_unit_name, sizeof(msg->hardware_unit_name));
...
{
    UhdpCaptureControl capctrl;
    memset(&capctrl, 0, sizeof(UhdpCaptureControl));
...
void Connection_Impl::queue_completed_scan(ScanObject_Impl* scan, bool force)
{
    // this scan has completed transmission, or is never going to receive scan
    // data. Add it to the output queue if it is the correct next output scan
    // sequence number, else insert it into the pending queue until the correct
    // next scan sequence number arrives. I.E. scans wait in the pending queue
    // when they are out of order. Their stay in this queue should be very
    // brief. When the out-of-order scan arrives, the entire pending queue
    // should be flushed to the output queue in sequence number order.
    ...
    if (seq < expected)
    {
        // the scan must be dropped, it's already been assumed missing or this
        // is somehow a duplicate
        scan->release();
        return;
    }
    else if (seq == expected || force)
    {
        // this is the expected next scan sequence number, append to output list
        scanlist_mutex.acquire();
        if (last_out_scan)
        {
        ...
        // every time a scan matching the expected scan sequence number is added
        // to the output list, check if scans sitting in the pending list can be
        // flushed. This is potentially recursive
        if (first_pending_scan &&
            first_pending_scan->scan_info.scan_sequence_number == expected)
        {
            scan = first_pending_scan;
            first_pending_scan = first_pending_scan->next_scan_object;
    ...
    // insert into the pending scan list in scan sequence order
    if (first_pending_scan)
    {
        if (seq < first_pending_scan->scan_info.scan_sequence_number)
        {
            // prepend
            scan->next_scan_object = first_pending_scan;
            first_pending_scan = scan;
        }
        else if (seq > last_pending_scan->scan_info.scan_sequence_number)
        {
            // append
            last_pending_scan->next_scan_object = scan;
            last_pending_scan = scan;
        }
        else
        {
            // insert
            ScanObject_Impl* cur = first_pending_scan;
            while (seq < cur->next_scan_object->scan_info.scan_sequence_number)
            {
                cur = cur->next_scan_object;
            }
            scan->next_scan_object = cur->next_scan_object;
            cur->next_scan_object = scan;
        }

        // we do not want to allow a missing scan info from causing permanent
        // blockage, so we control the maximum depth of the pending list.
        while (last_pending_scan->scan_info.scan_sequence_number >
               first_pending_scan->scan_info.scan_sequence_number + 8)
        {
            scan = first_pending_scan;
            first_pending_scan = first_pending_scan->next_scan_object;
            queue_completed_scan(scan, true);
        }
    }
    else
    {
        first_pending_scan = last_pending_scan = scan;
    }
}
...
void Connection_Impl::poll_socket_internal()
{
    ...
    for (;;)
    {
        ...
        else if (uhdp->message_type == UHDP_TYPE_FLUSH)
        {
            ...
            // flush pending scans, even if some are missing
            while (first_pending_scan)
            {
                ScanObject_Impl* scan = first_pending_scan;
                first_pending_scan = first_pending_scan->next_scan_object;
                queue_completed_scan(scan, true);
            }
            last_pending_scan = NULL;
        ...
        if (cur_pkt_counter + (ADDITIONAL_WINDOW / 2) > current_window)
        {
            current_window += ADDITIONAL_WINDOW;
...
class Connection*   make_connection(uint32_t radar_ip, uint32_t local_ip, uint32_t timeout_sec, UserLogAgent& log_agent, bool use_thread)
{
    ...
    // TODO: last_err = MCE_INVALID_RADAR_IP, MCE_INVALID_LOCAL_IP

    for (uint32_t ver = MAX_SUPPORTED_UHDP_VERSION; ver >= MIN_SUPPORTED_UHDP_VERSION; ver--)
    {
        int ret = con->send_hello(radar_ip, local_ip, timeout_sec, ver);
        if (ret == 0)
        {
...
bool run_warmup_scans(Connection& con)
{
    ...
    UhdpCaptureControl capctrl;
    memset(&capctrl, 0, sizeof(capctrl));
...
Connection* make_good_connection(uint32_t radar_ip, uint32_t local_ip, UserLogAgent& log_agent, bool use_thread)
{
    for (uint32_t i = 0; i < REBOOT_RETRIES; i++)
    {
        ...
        {
            con = make_connection(radar_ip, local_ip, 2, log_agent, use_thread);
            if (con)
            {
                break;
            }
