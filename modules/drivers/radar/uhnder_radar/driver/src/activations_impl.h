...
public:
    ...
    virtual ActivationIterator* make_iterator(float thresh_snr) const;
...
class ActivationIterator_Impl : public ActivationIterator
{
public:

    ActivationIterator_Impl(const Activations_Impl& parent, float thresh)
        : myact(parent)
        , cur_bucket(0)
        , cur_activation(0)
        , cur_vrx(0)
    ...
    virtual bool get_sample(float& snr,
                            float& range,
                            float& doppler,
                            float& azimuth,
                            float& elevation);
    ...
    const Activations_Impl& myact;

    uint32_t cur_bucket;
    uint32_t cur_activation;
    uint32_t cur_vrx;
    float    thresh_snr;
};
