...
#include "modules/drivers/radar/uhnder_radar/driver/system-radar-software/env-uhnder/coredefs/engine-defs.h"
...
#include "modules/drivers/radar/uhnder_radar/driver/src/file_xfer.h"
...
bool send_with_retry(int fd, sockaddr_in raddr, char* message, int len, size_t sz, uint32_t& retry_count)
{
    ...
    for (rs = 0; rs < MAX_RESENDS; rs++)
    {
        ...
        else if (*(uint16_t*)message == htons(TFTP_OP_ACK))
        {
            return true;
        }
        else
...
bool Connection_Impl::tftp_upload(const char* local_fname, const char* radar_fname)
{
    ...
    {
        printf("Local file %s too large for TFTP (32MB)\n", local_fname);
        fclose(fp);
    ...
    char msgbuf[MAX_PAYLOAD_BYTES + 8];
    ...
    *opcode = htons(TFTP_OP_WRITE_REQ);
    ...
    uint16_t block_id;
    for (block_id = 0; block_id < sizebytes / MAX_PAYLOAD_BYTES; block_id++)
    {
        *opcode = htons(TFTP_OP_DATA);
        *block_id_ptr = htons(block_id + 1);
        memcpy(payload, data + block_id * MAX_PAYLOAD_BYTES, MAX_PAYLOAD_BYTES);
        if (!send_with_retry(fd, raddr, msgbuf, MAX_PAYLOAD_BYTES + sizeof(*opcode) * 2, sizeof(msgbuf), counters.tftp_retransmissions))
        {
    ...
    remainder = sizebytes - block_id * MAX_PAYLOAD_BYTES;
    *opcode = htons(TFTP_OP_DATA);
    *block_id_ptr = htons(block_id + 1);
    memcpy(payload, data + block_id * MAX_PAYLOAD_BYTES, remainder);
    if (!send_with_retry(fd, raddr, msgbuf, remainder + sizeof(*opcode) * 2, sizeof(msgbuf), counters.tftp_retransmissions))
    {
        last_err = CON_TFTP_SEND_ERROR;
        goto upfail;
    }
