    ...
    case UHDP_TYPE_RDC2:
        {
            ...
            if (!rdc2_skewers)  // First RDC2 packet of scan
            {
                ...
                rdc2_summaries = new RHAL_RDsummary[hdr->total_tuple_in_scan];
                cur_rdc2_skewer = 0;
                cur_rdc2_summary = 0;
            }
...
bool            MUSICData_Impl::deserialize(ScanSerializer& s)
{
    ...
    if (len)
    {
        ...
        if (total_rdc2_rdbin * per_rdbin == len)
        {
            ...
            if (len == sizeof(RHAL_RDsummary) * total_rdc2_rdbin)
            {
                rdc2_summaries = new RHAL_RDsummary[total_rdc2_rdbin];
                if (s.read_scan_data_type(rdc2_summaries, sizeof(RHAL_RDsummary), total_rdc2_rdbin))
                {
                    keeper = true;
                }
            }
