...
#include "modules/drivers/radar/uhnder_radar/driver/system-radar-software/misc/rapidjson/document.h"
#include "modules/drivers/radar/uhnder_radar/driver/system-radar-software/misc/rapidjson/prettywriter.h"
#include "modules/drivers/radar/uhnder_radar/driver/system-radar-software/misc/rapidjson/stringbuffer.h"
...
#define DO_FLOAT(var)  if (t.HasMember(#var) && t[#var].IsDouble()) mythresh->var = t[#var].GetDouble(); else mythresh->var = 0.0f
        DO_FLOAT(static_1D_thresh_snr_dB);
        DO_FLOAT(moving_1D_thresh_snr_dB);
        DO_FLOAT(static_2D_thresh_snr_dB);
        DO_FLOAT(moving_2D_thresh_snr_dB);
        DO_FLOAT(scale_det_thresh_max_range);
        DO_FLOAT(scale_det_thresh_snr_adj);
        DO_FLOAT(base_static_snr_dB);
        DO_FLOAT(base_moving_snr_dB);
        DO_FLOAT(notch_width_radians);
        DO_FLOAT(notch_depth_dB);
        DO_FLOAT(outer_depth_dB);
        DO_FLOAT(ridge_threshold_dB);
        DO_FLOAT(phased_array_azimuth);
#undef DO_FLOAT
    }
    ...
    // Not loading "sra_version": "scc-v0.4.18-Au+242-3e2e73f6cfe7"
...
#undef DO_INT64
    ...
    for (uint32_t d12 = 0; d12 < 2; d12++)
    {
        ...
        {
            dc.PushBack(scan_info.dc_bias[d12][rx].i, allocator);
            dc.PushBack(scan_info.dc_bias[d12][rx].q, allocator);
        }
    }
...
#undef DO_FLOAT
    ...
    d.AddMember("sra_version", StringRef(get_sra_version_string()), d.GetAllocator());
...
}_data_type(output, strlen(output), 1);
}
