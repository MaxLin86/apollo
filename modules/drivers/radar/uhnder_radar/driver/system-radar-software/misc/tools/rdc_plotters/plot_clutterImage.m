...
function [clutImgOut htImgOut ] = plot_clutterImage(...
    filestr, ...
...
if ~exist('savePolarPlot','var')
    savePolarPlot = 1;
end
if ~exist('doPrintInterpPeaks','var')
    doPrintInterpPeaks=1;
end
...
% disable peak interp if plotting the doppler slice is enabled
if plotSliceDopBins
    disp('Disabling Peak interp for doppler static slice plot.')
    doPrintInterpPeaks=0;
end

% all sorts of hacky plotting goodness
detCircleMinSize = 20;   % Minimum size of a moving detection indicator circle
bigDetSNR = 20;     % Minimum SNR (in dB) for drawing velocity vectors
...
swamperDegToStartCensor = 10; % used by primalSidelobeDetFilter
plotSuppressedDets = 1; % plots suppressed dets in black
mitigateAngleSL = 0; % takes out most angle sidelobe dets
mitigateMIMOdets = 0;
mitigateDCdets = 0;
mitigateExtraSSdets = 0;
CaxisRngDb = 8;
...
if f>0
    ...
    % safe method for converting CIFMT 26 to a 3D clutter image matrix
    % co = reshape(clutImg, 16, 8, []);
    % ra = permute(co, [3, 2, 1]);
    % save('ra2d', 'ra', ra);
    ...
    if hasHeight
        ...
        heightImg = heightImg(:,detection_params.rangeGatesMid>=0);
        hImgY = hImgY - sum(detection_params.rangeGatesMid<0);
        fclose(f);
    end
    if hasDoppler
        %% Read Doppler Image
        ...
        dopplerImg = dopplerImg(:,detection_params.rangeGatesMid>=0);
        dImgY = dImgY - sum(detection_params.rangeGatesMid<0);
        fclose(f);
    ...
    if is2DMIMO % ToDo check which formats we have a hdpeaks file with new CI scheme
        ...
        hdpeaks = hdpeaks(:,detection_params.rangeGatesMid>=0);
        dImgY = dImgY - sum(detection_params.rangeGatesMid<0);
        fclose(f);
    ...
    if hasHeight
        ...
        elBinsDeg = unique(rad2deg(detection_params.angleGatesMid(:,2))); % get elevation bins
        adjhbin = min(heightImg ,length(elBinsDeg));
        angleElevDeg = interp1(elBinsDeg,max(adjhbin,0)+1)+ elevationAngleAdjust_deg;
        rngbinsm = detection_params.rangeGatesMid(detection_params.rangeGatesMid>=0);
        azbinsdeg = unique(rad2deg(detection_params.angleGatesMid(:,1)));
    ...
    if fmt == 21 % M16Z
        ...
        ym=detection_params.rangeGatesMid(detection_params.rangeGatesMid>=0); %(0:ysize-1)*pixel_meters;

        if fmt == 4 || fmt == 7 || fmt == 25 % range/angle plot
            if(~noNewFigs), figure; end
            ...
            if  fmt == 25
                assert(xsize==size(unique(detection_params.angleGatesMid(:,1)),1), ['Expected ' num2str(xsize) ' Azimuth Bins'])
            ...
            if ~polarplot
                ...
                if doPrintInterpPeaks
                    peaksStr = printInterpPeaksCI(clutImgOut, dBpeakThresh, peakCensorRngAzEl, xm, ym);
                    subplot(1,5,5)
                    axis off;
...
%                             plot3(rad2deg(detection_params.angleGatesMid(:,1)),detection_params.angleGatesMid(:,2),detection_params.dopplerGatesMidFFT(staticSliceBinIndices(:,ceil(size(staticSliceBinIndices,2)/2))),'.-'), xlabel('Az Angle (deg)'),ylabel('El Angle (deg)'), zlabel('Velocity (mps)');
                            centerEl=(5*16+1):6*16; plot(rad2deg(detection_params.angleGatesMid(centerEl,1)),detection_params.dopplerGatesMidFFT(staticSliceBinIndices(centerEl,ceil(size(staticSliceBinIndices,2)/2))),'.-'), xlabel('Az Angle (deg)'), ylabel('Velocity (mps)');
                        else
                            plot(rad2deg(detection_params.angleGatesMid(:,1)),detection_params.dopplerGatesMidFFT(staticSliceBinIndices(:,ceil(size(staticSliceBinIndices,2)/2)))), xlabel('Az Angle (deg)'), ylabel('Velocity (mps)');
                        end
                        fclose(f2);
                    end
                    xlabel('Angle(deg)');
                ...
                if enableHeightSurface && is2DMIMO
                    surface( ym, rad2deg(xm), filtheight, toplot2);
                    zlabel('Height(m)');
                    grid on;
                else
                    surf( ym, rad2deg(xm), toplot2,'LineStyle','none');
                end
                ...
                toplot2(toplot2<median(toplot2(:))-30)=median(toplot2(:))-30;
                [t,r] = meshgrid(xm,ym);
                [x,y] = pol2cart(t,r);

                % Define some angular and radial range vectors for example plots
                anglims = [min(xm) max(xm) ];
                rnglims = [minrng maxrng];
                ylim2 = ceil(rnglims(2)*sin(max(abs(xm))));
                ...
                if (enableHeightSurface || transparentCmap) && is2DMIMO
                    if transparentCmap
                        polarplot3d(filtheight.','plottype','alphasurf','radialrange',[minrng maxrng],...
                            'angularrange',[xm],'tickspacing',25,'colordata',toplot2.');
                    else
                        polarplot3d(filtheight.','plottype','surfn','radialrange',[minrng maxrng],...
                            'angularrange',[xm],'tickspacing',25,'colordata',toplot2.');
                    end
                else
                    polarplot3d(toplot2.','plottype','surfn','radialrange',[minrng maxrng],...
                        'angularrange',[xm],'tickspacing',25);
                end
                ...
                if rangeNorm && useSVAcolormap
                    c(1,:) = [0 0 .4];
                    colormap([c(1,:);c(25:end,:)]);
                    % caxis([c1 c2]);
                    % used for DeNoising plots for CES and Ford
                    caxis([median(toplot2(:))+7 max(max(toplot2(:))-3,median(toplot2(:))+CaxisRngDb)]);
                else
                    caxis([median(toplot2(:))+2 max(max(toplot2(:))-3,median(toplot2(:))+CaxisRngDb)]);
                end
                ...
                % %hax
                % zlim([-10 surfheight])
                % title(['Height map below ' num2str(surfheight) ' meters'])
                % ax=gca;
                % ax.Color=[0 0 .3];
...
%                     view([-81 70])
                    view([-14 10])
                elseif enableHeightSurface && is2DMIMO
                    zlabel('Height (m) dB');
                    hold on;
                    ...
                    zlabel('Mag (dB)');
                    zlim([median(toplot2(:))-20 max(toplot2(:))+3]);
                end
                    ...
                    if ~isempty(det)
                        censorDetMask = zeros(size(det.snr));
                            ...
                            if mitigateAngleSL
                                ...
                                detrangebin(detrangebin>127)=127;
                                ...
                                adjSNRthresh = 1; %only censor if adj RD det is 6dB below currDet
                                for RDit=1:length(RDinduni) %cycle through unique RD detections
                                    ...
                                    censorDetMask(tempind(tempaz>tempaz(indmax)+swamperDegToStartCensor | tempaz<tempaz(indmax)-swamperDegToStartCensor))=1;

                                    AdjacDetMask = ismember(RDind,RDindA(:,ia(RDit)).') & snrMax>(det.snr+adjSNRthresh); % in adjacent RDbin
                                    censorDetMask = censorDetMask | AdjacDetMask;
                                end
                            end
                            if mitigateMIMOdets
                                abins_deg = rad2deg(detection_params.angleGatesMid(:,1));
                                [staticSlicesOut, ~, ~, ~, ~] = plot_staticSliceImgSD(ssfile,[],0,2,0);
%                                 % TEMP hack for SW static detecion, bug do not check in
%                                 det.snr(det.staticFlag)= det.snr(det.staticFlag)-swExponent.RDC3*6;
%                                 det.mag(det.staticFlag)= det.mag(det.staticFlag)-swExponent.RDC3*6;
                                %tempnoise = det.mag-det.snr;
                                prefilteredSS=sort(abs(staticSlicesOut(:,:)));
                                evalVec = mag2db(prefilteredSS(round(0.9*size(prefilteredSS,1)),:)).';
                                interppts = interp1(abins_deg,evalVec,rad2deg(det.az));
                                badDetMask = logical(det.mag<=interppts & det.snr<(max(det.snr)-40));
%                                 figure, plot(abins_deg,evalVec,'.-')
%                                 hold, plot(rad2deg(det.az),det.mag,'o');
%                                 pause(2), close;
                                censorDetMask = censorDetMask | badDetMask;

                            end

                            if mitigateDCdets && abs(egoMotion.velocity.x)>0
                                % remove DC bias induced detections
                                censorDetMask = censorDetMask | det.dopp==0;
                            end
                            if mitigateExtraSSdets && abs(egoMotion.velocity.x)>0
                        ...
                        if  plotMovingDets && any(dynFlag)
                            ...
                            if PMCW.MIMO2D
                                ...
                                scatter3(...
                                    ...
                                    min(1000,max(detCircleMinSize, (1 .* (det.snr(plotflag) - 20)) .^ 2)), ...
                                    colormatrix(plotflag,:), ...
                                    'LineWidth',3, ...
                                    'Marker','o');
                            else
                            ...
                            if plotSuppressedDets
                                ...
                                scatter3(...
                                    ...
                                    det.range(aplotflag) .* sin(det.az(aplotflag)), ...
                                    ones(1,sum(aplotflag==1)) .* max(toplot2(:)), ...
                                    min(1000,max(detCircleMinSize, (1 .* (det.snr(aplotflag) - 20)) .^ 2)), ...
                                    [.25,.25,.25], ...
                                    'LineWidth',1.5, ...
                                    'Marker','o');
                            end
                            ...
                            if any(plotbigflag)
                                ...
                                quiver3(...
                                    det.range(plotbigflag) .* cos(det.az(plotbigflag)), ...
                                    det.range(plotbigflag) .* sin(det.az(plotbigflag)), ...
                                    ones(1,sum(plotbigflag==1)) .* max(toplot2(:)), ...
                                    real(nn), ...
                                    imag(nn), ...
