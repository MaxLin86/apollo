...
if ~isempty(strfind(a2,'rdc1'))
    ...
    [GenTestData, ~] = plot_RDC1data(RDC1binFile,RDC1expFile,inputData.RangeBins,inputData.Nvrx,inputData.DopplerBins,inputData.allowPlots);
    
    % Check Data
    noisemean_dB = mag2db(mean(abs(reshape(GenTestData(inputData.noiseRbins,:,:),1,[]))));
    targData_lin = GenTestData(inputData.targRbin,:,:);
...
elseif ~isempty(strfind(a2,'stsli'))
    ...
    stslindFile = [a1 '/' a2(1:mark) 'stslind.bin'];
    [GenTestData ] = plot_staticSliceImg(stslbinFile,stslindFile,inputData.RangeBins,inputData.DopplerBins,inputData.Nangle,inputData.halfWidthDopBins,inputData.loadStaticSliceExponents,inputData.allowPlots);
...
elseif ~isempty(strfind(a2,'hist'))
    histdatFile = [a1 '/' a2(1:mark) 'hist.bin'];
    histbmFile = [a1 '/' a2(1:mark) 'histbm.bin'];
    [GenTestData,~,~] = plot_histograms(histdatFile,histbmFile,1:inputData.RangeBins,inputData.allowPlots);
