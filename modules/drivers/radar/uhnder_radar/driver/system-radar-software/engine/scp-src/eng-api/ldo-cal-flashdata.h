...
struct LDOCalKey
{
    ...
    void upgrade_from_version(uint32_t old_version, struct LDOCalData& data)
    {
    }

    void uhprint() const
    {
        UHPRINTF("(LDO Cal) tx_power_mode: %d", tx_power_mode);
        UHPRINTF("            temperature: %.1f", temperature);
