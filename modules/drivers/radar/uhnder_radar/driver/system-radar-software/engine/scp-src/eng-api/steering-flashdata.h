...
#if __SCP__
    void upgrade_from_version(uint32_t old_version, struct SteeringData&)
    {
    }
#endif
...
struct SteeringData
{
    ...
    uint32_t    az_vrx_spacing;                         // Spacing (1/2 Lambda units ) between array elements, in azimuth (Y)
    uint32_t    el_vrx_spacing;                         // Spacing (1/2 Lambda units ) between array elements, in elevation (Z)
